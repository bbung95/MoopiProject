						...
						<table
							...
							<tbody>
								<c:forEach items="${list}" var="member">
									<tr>
										<td id="td1"><img class="userProfile"
											src="/images/uploadFiles/${member.mmUser.profileImage}"></td>
										...
										<td id="td7"><c:if test="${member.memberRole eq '4'}">
				모임장
				</c:if> <c:if test="${member.memberRole eq '3'}">
				운영진
				</c:if> <c:if test="${member.memberRole eq '2'}">
				멤버
				</c:if></td>
										<td id="td8">
											<%-- 			<c:if test = "${member.memberRole eq '2'}"> --%> <%-- 			<button type="button" id="${member.mmUser.userId}" class="btn btn-primary" onClick="fncMember1('${member.mmUser.userId}', '${member.mmNo}')">매니저임명</button> --%>
											<%-- 			</c:if> --%> <%-- 			<c:if test = "${member.memberRole eq '3'}"> --%>
											<%-- 			<button type="button" class="btn btn-warning" onClick="fncMember2('${member.mmUser.userId}', '${member.mmNo}')">매니저박탈</button> --%>
											<%-- 			</c:if> --%> <%-- 			<button type="button" class="btn btn-danger" onClick="fncMember3('${member.mmUser.userId}', '${member.mmNo}')">블랙</button> --%>
											
											<c:if test="${check.memberRole eq '4' || check.memberRole eq '3'}">
											<c:if test="${dbUser.userId == moim.mmConstructor.userId}">
